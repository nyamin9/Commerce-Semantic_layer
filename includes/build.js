...
//   P15  PTD 는 채운 grid 위에서 만든다. 빈 날을 두면 rollup 했을 때 무너진다
//   P17  기계적인 것은 생성한다
//   P18  틀린 결과를 내느니 거부한다
//
...
// 서빙 테이블의 dimension에는 NULL 을 두지 않는다. daily_ 의 NULL 은 '(unknown)' bucket이
...
//   =                     통과
//
// '(all)' 과도 겹치지 않는다. '(unknown)' 은 값이 없는 bucket이고 '(all)' 은 그 축을
// rollup 한 행이다
...
function resolveDims(name, m) {
  ...
  return dims.map((d) => {
    ...
    if (def.via && !(def.via in (e.joins || {}))) {
      throw new Error(
        `[${name}] dimension '${d}'의 via '${def.via}'가 joins에 없다. ` +
...
// 수식이 참조한 조인 이름. dimension이 안 쓰는 조인이라도 여기 나오면 붙여야 한다
...
// MERGE 는 지우지 않는다. dim 속성이 바뀌면 (record_date, dimension) 키가 달라져서 옛 행이
...
// ── 합치는 함수 — additive 가 고른다. null 이면 생성 거부 (P18) ─
...
// dimension 축을 합칠 때는 시간 축이 아니라 그 dimension 의 가산성이 함수를 고른다
...
//
// ── grid 가 핵심인 이유 ──────────────────────────────────────
// 활동한 날에만 PTD 를 만들면 rollup 하는 순간 대부분이 사라진다 — 실측으로 country 별
// MTD 가 실제의 13% 였다. 그날 안 팔린 조합의 앞 구간 매출이 통째로 빠지기
// 때문이다. 행을 만들어 두면 누적값이 앞 구간을 그대로 들고 간다.

// base — serving_dims 밖의 dimension 을 없앤다
...
// 기준 날짜 목록는 sem_dim_date 다. daily_ 의 날짜를 쓰면 전사적으로 거래가 0인 날이
// 통째로 빠져 누계의 연속성이 끊긴다 — 실측으로 2,811일 중 44일이 그랬다.
// 빈 날짜를 행으로 만드는 것이 이 dimension 테이블의 존재 이유다 (P15-1).
//
...
//
//   mask 0b1111  country  age_group  gender  acq      base
//   mask 0b0001  country  (all)      (all)   (all)    country 별 rollup
//   mask 0b0000  (all)    (all)      (all)   (all)    전사
//
// 진짜 NULL 은 NULL 로 남는다. 마스크가 그 축을 살린 행에서는 원본 값이 그대로
// 오므로 "값이 없는 bucket"(P6-1)과 '(all)' 이 섞이지 않는다.
//
// 합치는 함수는 dimension 축 가산성이 고른다 (P9). 완결 플래그는 record_date 에서
