...
`includes/metrics.js` 는 이 문서의 기계 판독 가능한 형태이고, **이 문서가 정본임.**

용어는 [glossary.md](glossary.md) 를 따름.

- **기본 지표 17개** — 직접 집계됨. `daily_` + `period_` + `metric_` 세 테이블을 가짐
- **비율 지표 7개** — 기본 지표의 나눗셈임. registry에만 등록하고 테이블을 만들지 않음 (P12)

---

## 1. entity와 dimension

**지표를 산출하는 fact 테이블이 entity가 됨.** entity가 정해지면 grain과
사용 가능한 dimension이 따라서 정해짐.
...
`purchase_type` 만 fact 를 조인해서 옴. `sem_fct_orders` 에서 한 번 계산하고
`order_item` 은 `order_header` 라는 이름으로 가져옴 — `order` 는 GoogleSQL 예약어라
조인 이름으로 못 씀 (P5-1). `order_key` 가 유일해서 fan-out 이 생기지 않음.

**`serving_dims` 는 이 표에서 뽑아낸 것임.** `period_`·`metric_` 은 entity 별로
아래 축만 갖고, 각 축의 `'(all)'` rollup 행까지 테이블로 저장함 (P4·P15).
...
지표별로 덮어쓸 수도 있음. `metrics.js` 에 `serving_dims` 를 적으면 그 지표의 큐브만
좁아지고 `daily_` 는 넓게 남음 — dimension 하나가 행 수를 곱하는 곳은 `period_` 이고
`daily_` 는 거의 안 커지기 때문임.
...
있음. grid가 조합 수만큼 부풀어서 서빙 테이블에 올릴 수 없음.

**`order`에 상품 dimension이 비어 있는 것은 누락이 아님.** 한 주문이 여러 상품을
포함하므로 주문 grain에서 카테고리는 정의되지 않음. 이 공백이 P10의 근거임.
...
결과 — dimension이 평범한 컬럼으로 테이블로 저장됨. **이 시점부터 조인은 더 필요 없음.**
...
> **테이블은 지표당 하나임. dimension 조합마다 생기지 않음.**
> 선언한 dimension 전부가 한 테이블의 컬럼으로 들어가고, 행은 `날짜 × 실제로 존재하는 dimension 조합`임.
...
`base` 는 `category`·`department`·`order_item_status` 를 없앰. rollup 행은 아직 없음.
...
`rollup` 이 마지막에 `'(all)'` 행을 만듦. 마스크를 `CROSS JOIN` 으로 붙여 입력을
한 번만 읽고 행을 2ⁿ 배로 펼침. 비트가 0인 축이 `'(all)'` 이 됨.
...
> **`CUBE` 도 `GROUPING SETS` 도 못 씀.** `CUBE` 는 다른 grouping element 와 섞이지
> 않고, `GROUPING SETS` 는 집합마다 입력을 다시 읽어 sketch 지표가 CPU 한도에 걸림
...
`grid` 는 `sem_dim_date` 를 기준 날짜 목록로 빈 날짜를 채움 (P15-1). 활동한 날에만 누계를
만들면 rollup 하는 순간 대부분이 사라짐 ([findings.md](findings.md) 4).

`cum` 은 가산이면 창 함수, sketch면 구간 자기조인임. **출력 컬럼은 양쪽이 같음.**
...
> `SUM` 으로도 틀린 값이 나옴. `SAFE_DIVIDE(SUM(v) - SUM(base), SUM(base))` 가
...
어떤 함수를 쓸지는 `additive` 가 정함. 사람은 고르지 않음.
...
sketch 는 합쳐도 sketch 로 남음. `MERGE` 로 정수를 만들면 더 합칠 수 없음 (P11).

> **`'(all)'` 행도 같은 규칙을 따름.** sketch 지표의 전사 값은 `SUM` 이 아니라
...
dimension 축에 `false`를 쓰게 되면 기록하고 넘어갈 사실이 아니라 **고쳐야 할 신호**임.
현재 기본 지표 17개에는 `false`가 하나도 없음. `order_count`를 `order` entity로
옮기면서 마지막 하나가 사라졌음.

entity 의 dimension 중 하나라도 `additive` 에 없으면 `dataform compile` 이 실패함 (P19).
...
그래서 `uniform(entity, value)` 로 펼침. 축마다 가산성이 다를 때만 직접 씀.
...
1. **정의가 하나임.** `net_revenue`가 무엇인지가 `metrics.js` 한 줄에만 있음.
   이것이 semantic layer의 존재 이유이고 나머지는 부수 효과임
2. **daily 이후 조인이 없음.** 소비 경로가 단순해지고 빨라짐
3. **대가 — 선언한 dimension으로만 자를 수 있음.** 새 dimension은 선언 추가와 재생성이 필요함

속도만 목적이라면 집계 캐시로 충분함. 이 구조는 1번을 위한 것임.

> 예시 쿼리는 `semantic_mart`가 아직 없어 DW 테이블에 직접 실행한 결과임.
> 마트를 만든 뒤에는 컬럼명이 규칙에 맞게 바뀜 — `category_name` → `category`.
...
양쪽에 다 있는 사람이 통째로 사라짐 ([findings.md](findings.md) 15). 그래서
`buyer_count` · `paying_buyer_count` · `void_buyer_count` 를 따로 만듦.

**세 값을 더해도 전체가 되지 않음.** HLL 근사 때문이 아니라 distinct count 의
성질이라, 정확히 세도 마찬가지임.
...
테이블을 만들지 않음. 분자와 분모가 각각 기본 지표이고, 나눗셈은 소비 시점에 함 (P12).
...
**카테고리별 AOV는 정의가 성립하지 않음.** 도구의 한계가 아니라,
"카테고리별 주문 수"라는 값이 존재하지 않기 때문임 — 한 주문이 여러 카테고리에
걸치므로 카테고리별로 나눠 세면 합이 전체 주문 수를 넘음.

이걸 굳이 내고 싶다면 분모를 다시 정의해야 함.
예를 들어 "해당 카테고리 상품을 포함한 주문 수"는 계산 가능하지만
**`order_count`와 다른 지표**이므로 별도 이름으로 선언해야 함.

`revenue_per_buyer`는 분모가 HLL 근사라 결과도 근사임. registry에 표시함.

---

## 5. 정의하지 않는 지표

빠진 것이 아니라 **의도적으로 제외**한 것임. 이유를 남김 (P17 — 등록과 생성은 다름).
...
generator 밖에서 별도 모델로 만들되 registry에는 `custom: true`로 등록함.

---

## 6. 산출물

컬럼과 타입은 [tables.md](tables.md) 에 있음. 여기서는 각 테이블의 역할만 다룸.
...
dimension 이 `serving_dims` 로 좁혀지므로 `category`·`department`·`order_item_status` 는 여기 없음. 그것이
필요하면 `daily_` 에서 직접 집계함 (P4). 대신 남은 4개 dimension 의 `'(all)'` rollup 행이 테이블로 저장돼 있음.
...
**dimension 을 rollup 하며 `SUM` 하지 않음.** `'(all)'` 행을 읽음 — 이미 테이블로 저장돼 있고,
...
`daily_` 에서 직접 구간 합을 내는 방법도 여전히 유효함 — `category` 처럼
서빙 테이블에 없는 축으로 누계를 봐야 할 때가 그러함.
...
선언이 JS 파일에만 있으면 어떤 쿼리로도 읽을 수 없음.
registry가 있으면 "`net_revenue`가 무엇인가"를 SQL로 답할 수 있음.
...
**비율 지표 7개는 테이블이 없으므로 registry가 유일한 거처임.**
5장의 제외 지표도 `is_generated: false`로 여기 남음 — "생성되지 않았다"와
"존재하지 않는다"는 다름 (P17).
...
전부 `true`임. **하나라도 `false`가 나오면 2단계로 돌아감** (P10).

> **`buyer_count`는 3단계에서 걸렸음.**
>
> 구매자 수는 어제도 구매하고 오늘도 구매한 사람이 있을 수 있음. 그래서 날짜축으로
> 더하면 같은 사람을 두 번 셈 — 날짜축 비가산임.
>
...
`uniform` 이 `entity` 의 dimension 전체에 같은 가산성을 펼침. 축마다 다르면
객체를 직접 쓰고, 그때 빠진 축이 있으면 컴파일이 실패함 (P19).
...
**지표 추가 비용이 선언 한 항목으로 고정됨.** 이것이 이 구조의 이득 전부이고,
새 fact가 필요한 지표만 `entities.js`에 항목이 하나 더 붙음.
