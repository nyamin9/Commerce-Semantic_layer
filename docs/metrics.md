...
- **기본 지표 17개** — 직접 집계됨. `daily_` 하나 + 조합마다 (`period_`, `metric_`) 한 쌍
...
- **지표를 산출하는 fact 테이블이 entity가 됨.** entity가 정해지면 grain과 사용 가능한 dimension이 따라서
  정해짐
...
- `sem_fct_orders` 에서 한 번 계산하고 `order_item` 은 `order_header` 라는 이름으로 가져옴 —
  `order` 는 GoogleSQL 예약어라 조인 이름으로 못 씀 (P5-1)
- `order_key` 가 유일해서 fan-out 이 생기지 않음

- **`serving_dims` 는 이 표에서 뽑아낸 것임.** `period_`·`metric_` 은 entity 별로 아래 축만 갖고,
  각 축의 `'(all)'` rollup 행까지 테이블로 저장함 (P4·P15)
...
- 결과 — dimension이 평범한 컬럼으로 테이블로 저장됨
- **이 시점부터 조인은 더 필요 없음.**
...
- `base` 는 `category`·`department`·`order_item_status` 를 없앰
- rollup 행은 아직 없음
...
- `rollup` 이 마지막에 `'(all)'` 행을 만듦
- 마스크를 `CROSS JOIN` 으로 붙여 입력을 한 번만 읽고 행을 2ⁿ 배로 펼침
- 비트가 0인 축이 `'(all)'` 이 됨
...
- sketch 는 합쳐도 sketch 로 남음
- `MERGE` 로 정수를 만들면 더 합칠 수 없음 (P11)
...
- 현재 기본 지표 17개에는 `false`가 하나도 없음
- `order_count`를 `order` entity로 옮기면서 마지막 하나가 사라졌음
...
- 직접 집계되는 지표
- 각각 `daily_` · `period_` · `metric_` 세 테이블을 가짐
- `daily_` 는 언제나 **그 entity의 dimension 전체**를 가짐 (1장 표)
- 지표가 좁힐 수 없음
...
- 매출은 `gross_revenue − net_revenue` 로 취소·반품분이 나옴
- 라인마다 한 bucket 에만 들어가기 때문임
- **구매자는 그 뺄셈이 안 됨.**

- 한 사람이 완료 주문과 취소 주문을 둘 다 가질 수 있어서 bucket 이 겹침
- 뺄셈을 하면 양쪽에 다 있는 사람이 통째로 사라짐 ([findings.md](findings.md) 15)
- 그래서 `buyer_count` · `paying_buyer_count` · `void_buyer_count` 를 따로 만듦

- **세 값을 더해도 전체가 되지 않음.** HLL 근사 때문이 아니라 distinct count 의 성질이라, 정확히 세도 마찬가지임

- `buyer_count − paying_buyer_count` 만 성립함
- `paying` 이 `buyer` 의 부분집합이라 그 차이가 "매출을 한 번도 내지 못한 고객" 이 됨 (12,752명)
...
- 테이블을 만들지 않음
- 분자와 분모가 각각 기본 지표이고, 나눗셈은 소비 시점에 함 (P12)
...
- **카테고리별 AOV는 정의가 성립하지 않음.** 도구의 한계가 아니라, "카테고리별 주문 수"라는 값이 존재하지 않기 때문임 — 한
  주문이 여러 카테고리에 걸치므로 카테고리별로 나눠 세면 합이 전체 주문 수를 넘음

- 이걸 굳이 내고 싶다면 분모를 다시 정의해야 함
- 예를 들어 "해당 카테고리 상품을 포함한 주문 수"는 계산 가능하지만 **`order_count`와 다른 지표**이므로 별도 이름으로
  선언해야 함

- `revenue_per_buyer`는 분모가 HLL 근사라 결과도 근사임
- registry에 표시함
...
- 빠진 것이 아니라 **의도적으로 제외**한 것임
- 이유를 남김 (P17 — 등록과 생성은 다름)
...
- 컬럼과 타입은 [tables.md](tables.md) 에 있음
- 여기서는 각 테이블의 역할만 다룸
...
- distinct 계열은 HLL sketch(BYTES)로 남음
- 소비용이 아니라 `metric_`의 재료임

### 6-2. `period_<metric>` — 기간 확장

- `daily_` 를 **`serving_dims` rollup × 기간 컬럼**으로 편 것
- 비교는 아직 없음
...
- 5장의 제외 지표도 `is_generated: false`로 여기 남음 — "생성되지 않았다"와 "존재하지 않는다"는 다름 (P17)
...
- 수량은 주문 라인 단위임
- 따라서 entity는 `order_item`, 소스 테이블은 `sem_fct_order_items`
...
- entity가 정해지면 쓸 수 있는 dimension이 [1장의 dimension 도달 경로
  표](#1-1-dimension-도달-경로-join-graph)로 결정됨
- 고를 여지가 없음
- `order_item`의 경우 이러함
...
- **고를 수 없음.** `daily_` 는 언제나 entity 의 dimension 전체를 가짐
- 지표가 `dims` 를 선언하면 컴파일이 거부함 (P4-1)
...
- `uniform` 이 `entity` 의 dimension 전체에 같은 가산성을 펼침
- 축마다 다르면 객체를 직접 쓰고, 그때 빠진 축이 있으면 컴파일이 실패함 (P19)

- **여기서 사람의 작업은 끝남.** 아래는 전부 생성됨

### 7-5. 5단계 — `daily_cancelled_units` (생성)

- `entity.source`를 base로, `joins` 선언만큼 `LEFT JOIN`, `date_col` + dimension으로
  `GROUP BY`
...
- dimension 8개가 전부 들어감
- 선언에 적지 않아도 `entities.js` 가 정함
...
- **지표 추가 비용이 선언 한 항목으로 고정됨.** 이것이 이 구조의 이득 전부이고, 새 fact가 필요한 지표만
  `entities.js`에 항목이 하나 더 붙음
