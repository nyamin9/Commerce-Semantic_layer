...
서빙 테이블을 세로(`period_type` 행)에서 가로(기간 컬럼)로 바꾼 변경의 설계 노트다.
구현과 검증이 끝났고, 결정은 `principles.md`(P4·P6-3·P13·P15)와 `metrics.md` 에 들어가 있다.
여기 남은 것은 **왜 그렇게 정했는가와 실측값**이다. 접어 넣을지는 별도로 정한다.
...
period_<metric>   record_date × 4축 + 롤업        daily·wtd·mtd·ytd 가로. 비교 없음
...
```
1) base    daily_ 를 serving_dims 로 접는다.              기저 조합만
2) grid    sem_dim_date × 조합 을 전부 만들고 값을 붙인다.  없으면 0 / NULL
3) cum     그 위에 누적한다.                              daily 는 그대로 통과
4) rollup  각 축의 '(all)' 행을 만든다.                    마스크 CROSS JOIN
```

**접기(4)가 누적(3)보다 나중인 것이 중요하다.** 순서를 바꿔도 값은 같다 — `SUM` 은
결합법칙이 성립하고 HLL 병합은 합집합이라 조합별 WTD 스케치를 합친 것이 전체 WTD
스케치와 같다. 비용은 전혀 다르다. 먼저 접으면 `'(all)'` 행의 스케치가 조밀해지는데,
스케치 누계는 1년 구간을 자기조인하므로 그 조밀한 스케치를 하루당 180여 번씩 읽는다 —
`buyer_count` 가 CPU **1,748,227초**를 써서 한도(5,100)에 걸렸다. 기저 조합에서
누적하면 스케치가 희소한 채로 조인된다.

**2번도 핵심이다.** 활동한 날에만 누계를 만들면 걷는 순간 대부분이 사라진다 — 실측으로
...
4번 단계가 롤업 행을 만들 때 쓰는 함수도 가산성이 정한다.
...
2026-09-16 전체 재생성 후 실측. 전부 통과했다.

| 검증 | 대상 | 불일치 |
|---|---|---|
| `daily` 컬럼이 `daily_` 를 4축으로 걷은 값과 같은가 | 1,635 | **0** |
| `'(all)'` 행이 각 축의 합과 같은가 | 260 | **0** |
| 월말 `mtd` 가 `daily_` 의 그 달 합과 같은가 | 20 | **0** |
...
```
2026-08 전사 MTD 구매자   스케치 10,264 / 원자 fact 직접 계산 10,267   오차 0.029%
'(all)' 행                스케치 10,264 / 국가별 행을 MERGE 10,264     정확히 일치
```

`period_`·`metric_` 은 각각 15개, `daily_` 15개까지 45개 테이블이 생성됐고
`semantic` 데이터셋은 13.8 GB · 1억 52만 행이다. `metric_net_revenue` 가
4,801,188 행 · 1,017 MB, 스케치 지표인 `metric_buyer_count` 가 1,666 MB 다.
