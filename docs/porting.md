# 다른 프로젝트로 옮기기

- 이 레포의 무엇이 어디서나 통하고 무엇이 이 데이터셋에만 해당하는지
- 용어는 [glossary.md](glossary.md) 를 따름

- 네 갈래로 나뉨

| | |
|---|---|
| **1. 설계** | 엔진과 데이터가 달라도 그대로 통함. 옮길 가치가 있는 것은 이쪽임 |
| **2. 선언** | 갈아끼움. 새 프로젝트의 fact 와 지표를 적으면 됨 |
| **3. 엔진** | BigQuery·Dataform 에 묶여 있음. 다른 엔진이면 다시 재야 함 |
| **4. 서빙 레이어** | Cube·Looker 위로 옮길 때 `serving_dims` 가 무엇이 되는가 |

- 5장은 옮길 때 처음에 정할 것을 모은 것임

---

## 1. 설계 — 어디서나 통하는 것

### 1-1. 테이블을 3단계로 나눔

```
1단계  일별 집계   조인을 실행한다. dimension 을 전부 갖는다
2단계  기간 확장   dimension 을 좁히고 PTD 와 rollup 행을 만든다
3단계  비교        2단계를 shift 해 self-join 한다
```

- **셋 다 테이블로 저장함.** 한 쿼리에 넣으면 같은 집계가 여러 번 돎
- 2단계를 3단계 안의 서브쿼리로 두면 self-join 수만큼 다시 계산됨

- **1단계를 따로 두는 이유는 조인임.** 2단계 이후는 1단계만 읽으므로 fact 와 dimension 을 다시 읽지 않음
- dimension 을 추가해도 fact 를 다시 만들 필요가 없음

### 1-2. 지표를 선언으로 쓰고 SQL 을 생성함

```
선언 1항목  →  테이블 3개 + 카탈로그 1행
```

- 사람이 쓰는 집계 SQL 이 0개가 되는 것이 목표임
- 손으로 쓰면 지표당 모델 여러 개를 만들고 비교 로직을 복사하게 되는데, 복사한 쪽과 원본이 어긋나도 에러가 나지 않음

- 선언에 담을 것은 이만큼임

```
entity      어느 fact 에서 나오는가
expr        집계식
filter      집계 전 행 필터 (선택)
dims        쓸 수 있는 dimension
additive    축별 가산성
```

### 1-3. `additive` 를 축별로 선언함

- 플래그 하나가 아니라 **축마다** 선언함
- 같은 지표라도 축에 따라 다르기 때문임

```
true      그 축으로 합산 가능
"sketch"  병합 가능한 중간 상태로 저장
"last"    스냅샷. 마지막 값
false     복원 불가
```

- 이 선언 하나가 rollup 함수와 PTD 계산 방법을 동시에 정함
- 사람이 `SUM` 인지 `MERGE` 인지 고르지 않음

- **dimension 축에 `false` 가 나오면 entity 가 틀린 것임.** 기록할 사실이 아니라 고칠 신호로 다룸

- 잘못 선언하면 `SUM` 이 선택되어 distinct count 가 부풂 — 에러는 나지 않고 숫자만 틀림
  ([findings.md](findings.md) 11)

### 1-4. entity 는 fact 이고 grain 은 바뀌지 않음

- fact 하나가 entity 하나임
- entity 가 정해지면 grain 과 쓸 수 있는 dimension 이 따라서 정해짐
- **한 entity 안에서 grain 을 섞지 않음.**

- 주문 grain 에 상품 dimension 이 없는 것은 누락이 아님
- 한 주문이 여러 상품을 포함하므로 그 grain 에서 category 가 정의되지 않음
- 이 빈자리가 "주문 수를 어느 entity 에 둘 것인가" 의 답이 됨

### 1-5. 서빙 grain 을 따로 둠

- 1단계는 dimension 을 전부 갖고, 2단계는 그 부분집합만 가짐

- 2단계가 전부를 못 쓰는 이유는 1-6 의 빈 날 채우기 때문임
- 행 수가 **(dimension 조합 수 × 날짜 수)** 로만 정해지므로 조합이 늘면 그대로 곱해짐

- 고르는 기준은 **고유값이 적고 entity 를 가로지르는 dimension**(conformed dimension)임
- 서로 다른 fact 의 지표를 나란히 놓을 수 있는 축이기 때문임

- 고유값이 많은 dimension 은 연 단위로 집계해도 행이 거의 줄지 않음
- 값이 2개인 dimension 도 조합을 3배로 만듦 ([findings.md](findings.md) 2)

**넣기 전에 셋을 확인함.**

| | 기준 |
|---|---|
| dimension 고유값 | 10개 이하 |
| 조합 수 | 1,500 내외 |
| distinct 근사 지표의 누계를 **실제로 돌려봄** | dry run 으로는 안 잡힘 |

- 세 번째가 핵심임
- 이 프로젝트에서 가장 아팠던 것이 전부 거기였고, 컴파일로도 dry run 으로도 미리 잡히지 않았음

### 1-6. PTD 는 빈 날을 채운 위에서 계산함

- 활동이 있는 날에만 PTD 행을 만들면 **rollup 했을 때 대부분이 사라짐.**

- 빈 날을 채우지 않으면 rollup 한 값이 실제의 13% 가 나옴
- 그날 활동이 없던 조합의 앞 구간이 통째로 빠지기 때문임 ([findings.md](findings.md) 4)

```
1) 날짜 × dimension 조합을 모두 만든다
2) 값을 붙인다. 없으면 0 (가산) 또는 NULL (sketch)
3) 그 위에 누적한다
```

- **날짜는 날짜 dimension 테이블에서 가져옴.** 집계 결과의 날짜를 쓰면 전사적으로 활동이 0인 날이 빠져 PTD 가 끊김

- **범위는 실제 데이터가 있는 구간으로 자름.** 그러지 않으면 미래 날짜에 행이 생김

### 1-7. rollup 보다 PTD 를 먼저 계산함

- 순서를 바꿔도 값은 같음
- 합산은 결합법칙이 성립하고 HLL 병합은 합집합임

- **비용은 다름.** rollup 을 먼저 하면 `'(all)'` 행의 sketch 가 조밀해지는데, sketch PTD 는 구간을
  self-join 해서 병합하므로 그 조밀한 sketch 를 날마다 수백 번 읽음

- rollup 을 먼저 하면 CPU 한도에 걸려 생성 자체가 실패함 ([findings.md](findings.md) 6)

### 1-8. 완결된 기간을 따로 만들지 않음

- `weekly`·`monthly`·`yearly` 는 PTD 와 값이 같음
- 기간의 마지막 날인지를 표시하는 플래그 하나면 됨

```
monthly  =  mtd  where is_month_end
```

- 행으로 따로 두면 값이 중복되고, 진행 중인 기간이 아직 오지 않은 날짜를 달게 됨
- 그 행의 비교 기준값은 4일치를 7일 전체와 맞대는 식으로 틀림

### 1-9. 비교는 증감률이 아니라 기준값을 저장함

- 비율을 저장하면 rollup 할 때 깨짐
- `AVG` 도 `SUM` 도 틀린 값을 냄
- shift 한 시점의 **값**을 복사해 두고 나눗셈은 조회 시점에 함

- **self-join 은 간격 단위로 묶음.** 비교 컬럼이 8개라도 서로 다른 간격이 5개면 조인은 5번임

- **주 단위 비교의 전년 shift 는 364일임.** 1년으로 shift 하면 요일이 어긋남

### 1-10. 선언 오류는 컴파일 타임에 거부함

- 런타임에 틀린 숫자가 나오는 것보다 생성을 거부하는 쪽이 나음

```
미선언 dimension 사용         예외
additive 키 누락              예외
dimension 축이 false          예외
조인 이름이 SQL 예약어        예외
지표 수식의 중괄호가 안 닫힘   예외
```

- 빈 테이블을 만들지 않는 것도 같은 이유임
- 빈 테이블이 남으면 소비자가 "값이 0" 으로 오해함

### 1-11. 증분은 구간을 지우고 다시 넣음

- `MERGE` 는 지우지 않음
- dimension 값이 바뀌면 키가 달라져 옛 행이 매칭되지 않고 그대로 남음
- 키는 여전히 유일하므로 `uniqueKey` 검사도 통과함

- 상류를 최신화하고 `MERGE` 증분을 돌리면 남은 옛 행이 합계를 부풀림 ([findings.md](findings.md) 9)

- **증분 구간은 상류가 정함.** 상류가 최근 N일을 덮어쓰면 우리도 N일을 다시 읽음
- 상류보다 촘촘하게 잡아도 이득이 없고, 넓게 잡으면 다시 읽기만 함

- **상류가 전체를 다시 만드는 fact 위에는 증분을 올리지 않음.** 과거 구간의 변경을 놓침

### 1-12. 상류에 건 검사는 gate 가 아니라 감시임

- 우리가 고칠 수 없는 것은 파이프라인을 멈추게 하지 않음
- 별도 워크플로로 돌려 보고만 함
- 알려진 결함은 우회하지 말고 기록함 — 조용히 우회하면 문제가 드러나지 않음

### 1-13. 나란히 적힌 선언은 구조로 묶지 말고 감시함

- 같은 값이 두 곳에 적히는데 한쪽이 다른 쪽을 읽을 수 없는 경우가 있음
- 이 레포에는 둘 있음

| 두 곳 | 어긋나면 |
|---|---|
| `entities.js` 의 `date_col` ↔ 마트의 `partitionBy` | 증분이 파티션을 못 걸러 느려짐. 값은 맞음 |
| 액션의 `tags` ↔ 실행 설정의 `includedTags` | 0개 액션으로 성공함. 알림도 없음 |

- **구조로 묶고 싶어지지만 방향을 보면 안 됨.** 마트가 `entities.js` 를 참조하게 만들면 상류가 하류를 읽게 되고, 마트를
  다른 팀이 소유하면 결합이 조직 경계를 넘음

- 대신 **양쪽을 다 읽을 수 있는 제3자가 대조함.**

```
파티션   assertion 이 INFORMATION_SCHEMA 와 entities.js 를 대조    gate
태그     apply.js 가 컴파일 그래프와 workflows.json 을 대조        적용 전 거부
```

- 옮길 때도 같은 원칙을 씀 — 선언을 합치기 전에 **누가 누구를 읽어야 하는지**를 먼저 봄

---

## 2. 선언 — 갈아끼우는 것

- 새 프로젝트에서 실제로 쓰는 파일은 이만큼임

| 파일 | 무엇을 적나 |
|---|---|
| `workflow_settings.yaml` | 프로젝트 ID · 리전 · `dataformCoreVersion` · 상류 데이터셋 |
| `includes/naming.js` 의 `DATASETS` | 우리가 만드는 데이터셋 3개. 각 파일은 `schema` 를 명시하고 값은 여기 |
| `includes/naming.js` 의 `TAGS` | 태그 이름. `infra/apply.js` 가 이것으로 `workflows.json` 을 검증함 |
| `definitions/sources/declarations.js` | 읽을 상류 테이블 목록 |
| `definitions/mart/*.sqlx` | 상류를 정규화한 중간 테이블. **사람이 SQL 을 쓰는 유일한 곳** |
| `includes/entities.js` | fact 마다 `source` · `pk` · `date_col` · `grain` · `joins` · `dims` · `serving_dims` · `refresh` |
| `includes/metrics.js` | 지표 목록 |
| `definitions/assertions/*.js` | 상류 감시 |
| `infra/workflows.json` | 스케줄 · 태그 · 실행 계정 |

**거의 그대로 쓰는 것**

| 파일 | 손볼 곳 |
|---|---|
| `includes/naming.js` | `DATASETS` · `TAGS` · 접두사 |
| `includes/periods.js` | 기간이 다르면. 대개 그대로 |
| `includes/build.js` | 없음 |
| `definitions/semantic/gen_*.js` | 없음 |
| `definitions/metadata/gen_registry.js` | 없음 |

### 2-1. 옮기는 순서

```
1. workflow_settings.yaml 과 declarations.js        상류를 읽을 수 있게
2. mart/*.sqlx                                      surrogate key 와 grain 을 보증
3. entities.js 에 entity 하나                        가장 중요한 fact 부터
4. metrics.js 에 지표 하나                           끝까지 돌려본다
5. serving_dims 를 정한다                            조합 수 × 날짜 수를 먼저 세어본다
6. 나머지 entity 와 지표를 채운다
7. infra/workflows.json                             스케줄
```

- **4번까지를 먼저 끝냄.** 지표 하나가 3단계를 모두 통과하는 것을 확인한 뒤에 나머지를 채우는 편이 빠름

- **5번에서 반드시 조합 수를 세어봄.** `serving_dims` 후보의 고유값을 곱하고 날짜 수를 곱하면 2단계 테이블의 행 수가
  나옴
- 이 숫자가 감당되는지가 설계의 분기점임

---

## 3. 엔진 — 옮기면 다시 재야 하는 것

- **이 절이 이 문서의 핵심임.** 컴파일로도 dry run 으로도 미리 잡히지 않는 것이 섞여 있음
- 숫자는 [findings.md](findings.md) 에 있음

### 3-1. BigQuery

| 제약 | 증상 | 대응 |
|---|---|---|
| **`CUBE` 를 다른 grouping element 와 못 섞음** | `GROUP BY record_date, CUBE(...)` 가 *"only supports CUBE when there are no other grouping elements"* 로 거부 | 부분집합을 직접 펼치거나 마스크를 씀 |
| **`GROUPING SETS` 가 집합마다 입력을 다시 읽음** | dimension 4개면 16번. 입력에 self-join 이 있으면 CPU 한도 초과 | 마스크를 `CROSS JOIN` 으로 붙여 입력을 한 번만 읽음 |
| **`IS NOT DISTINCT FROM` 이 해시 조인 키가 못 됨** | 일반 술어로 취급되어 중첩 루프가 됨. 구간 self-join 에서 CPU 한도 초과 | `NULL` 을 값으로 바꿔 `=` 로 조인함 |
| **`HLL_COUNT.MERGE_PARTIAL` 이 analytic function 을 지원하지 않음** | dry run 통과, 실행에서 `Analytic function MERGE_PARTIAL is not supported` | window function 대신 구간 self-join |
| **on-demand 의 CPU/바이트 비율 제한** | 스캔이 작아도 CPU 를 많이 쓰면 거부함 | 중간 결과를 테이블로 저장해 재계산을 없앰 |
| **파티션 컬럼을 바꿀 수 없음** | `CREATE OR REPLACE` 가 *"Cannot replace a table with a different partitioning spec"* 로 거부 | 테이블을 지우고 다시 만듦 |
| **예약어 78개** | 조인 이름이 `order`·`cube` 면 생성된 SQL 이 깨짐 | 컴파일 타임에 거부함 |
| **`DATE_SUB` 이 월말을 보정함** | `2026-03-31 - 1 MONTH = 2026-02-28`. 3/30 과 3/31 이 둘 다 2/28 로 감 | 월말 며칠의 전월 비교는 이 성질을 알고 봄 |
| **HLL precision** | 바꾸면 과거 sketch 와 병합할 수 없음 | 처음에 정하고 고정함. 이 레포는 15 |

**다른 엔진으로 옮길 때 확인할 것**

```
GROUPING SETS / CUBE 의 재계산 여부      → 마스크 방식이 필요한가
NULL-safe 조인이 해시 조인이 되는가      → '(unknown)' bucket 이 필요한가
distinct 근사 자료구조의 이름과 정밀도   → HLL 대신 무엇이 있는가
그 자료구조를 window function 에서 쓸 수 있는가   → 구간 self-join 이 필요한가
파티션 정의를 바꿀 수 있는가             → 스키마 변경 절차
```

### 3-2. Dataform

| 성질 | 무엇 |
|---|---|
| **CTE 는 결과를 저장하지 않음** | 여러 번 참조하면 그만큼 다시 계산됨. 단계를 테이블로 쪼개는 근거 |
| **`ctx.ref()` 가 의존 관계를 등록함** | builder 에 프로젝트 이름도 데이터셋 이름도 들어가지 않음 |
| **`preOps` 로 `DECLARE` 와 `DELETE` 를 넣음** | insert_overwrite 를 구현하는 방법 |
| **`uniqueKey` 를 주면 `MERGE` 를 씀** | 쓰지 않음. `preOps` 로 직접 구간을 지움 |
| **컴파일 타임 도구임** | 런타임에 SQL 을 조립하지 않음. 선언한 조합 안에서만 움직임 |
| **release / workflow configuration 은 git 에 없음** | GCP resource 임. 선언 파일을 따로 두고 스크립트로 맞춤 |

- **dbt 로 옮긴다면** — 3단계 분리는 모델 3개로, `additive` 선언은 매크로로, generator 는 Jinja 루프로
  바뀜
- 설계(A)는 그대로 통하고, CTE 재계산 문제도 같은 방식으로 나타남
- dbt 역시 컴파일 타임 도구라 자동 라우팅은 없음

---

## 4. 서빙 레이어가 있는 환경으로 옮긴다면

- `entities.js` 와 `metrics.js` 는 거의 그대로 옮겨감
- **`serving_dims` 만 성격이 바뀜.**

```
entities.js   →  Cube 의 cubes · joins · dimensions
metrics.js    →  Cube 의 measures
periods.js    →  granularity · 시간 비교 설정
serving_dims  →  대응 개념이 없다. preAggregations 선언이 된다
```

### 4-1. 사전 집계는 자동으로 만들어지지 않음

- Cube 든 Looker 든 **어떤 사전 집계를 만들지는 손으로 선언함.** 자동인 것은 선택임 — 쿼리가 들어오면 맞는 것을 고르고,
  없으면 원본으로 감

```js
// Cube. a×b 와 a×c 가 필요하면 두 블록을 직접 쓴다
preAggregations: {
  byCountryAndType: { measures: [...], dimensions: [CUBE.country, CUBE.purchaseType], ... },
  byCategory:       { measures: [...], dimensions: [CUBE.category], ... },
}
```

- `a × b × c × d` 의 모든 부분집합이 알아서 생기지는 않음
- 조합 폭발이라 어떤 도구도 그렇게 하지 않음
- 다만 실제 쿼리 로그를 보고 후보를 **추천**하는 기능은 있음 — Cube 의 Rollup Designer, Looker 의
  Aggregate Awareness Recommendations

### 4-2. 그래서 여러 개를 두는 편이 더 저렴함

```
사전 집계 하나   a × b × c × d       모든 조합. 이 레포의 방식
rollup 여러 개   a×b · a×c · b×d     실제로 쓰는 것만. 나머지는 원본으로 간다
```

- **fallback 이 있어서 다 안 만들어도 되는 것임.** 이 레포는 자동 라우팅이 없어서 안 만든 조합에 답이 없고, 그래서 전부
  만들어야 함
- dimension 하나가 행 수를 곱하는 이유가 여기에 있음

### 4-3. 이 레포에서 쪼개려면

- `metrics.js` 의 `rollups` 로 한 지표에 조합을 여럿 둘 수 있음
- 조합마다 `period_`·`metric_` 한 쌍이 생기고, 이름에 `__<조합>` 이 붙음
- **기본 조합은 대체되지 않고 남음** (P4-2)

- **다만 소비자가 어느 테이블을 읽을지 알아야 함.** 쿼리를 보고 골라주는 계층이 없어서 view 로도 못 가림 — view 는 들어온
  쿼리에 따라 대상을 바꾸지 못하고, `UNION` 으로 묶으면 전부 스캔해 쪼갠 의미가 없음

- 그래서 순서가 중요함
- **소비자에게 열기 전에 쪼개면 마이그레이션 비용이 0임.**

```
1. 넓게 만든다
2. 분석가 몇 명에게 먼저 연다          ← 쿼리 로그가 여기서 생긴다
3. INFORMATION_SCHEMA.JOBS 로 실제 조합을 센다
4. 그 근거로 쪼갠다
5. 전사에 연다
```

- 2번을 건너뛰면 4번을 감으로 하게 됨

- 다만 `rollups` 의 쓸모는 쪼개기보다 **기본 조합이 못 여는 축을 여는 것**임
- `category`(26값)처럼 고유값이 많아 `serving_dims` 에 못 넣는 축을, 좁게 묶으면
  기본 조합의 1~2% 크기로 열 수 있음

## 5. 옮길 때 처음에 정할 것 세 가지

- 가장 되돌리기 어려운 순서임

- **하나 — `serving_dims`.** 2단계 테이블의 행 수를 결정함
- 나중에 넓히면 재생성 비용이 크고, 좁히면 그 dimension 으로 보던 것을 잃음
- 조합 수를 미리 세어봄

- **둘 — distinct 근사의 정밀도.** 바꾸면 과거 데이터와 병합할 수 없음

- **셋 — 날짜 컬럼 이름과 파티션.** 파티션 컬럼은 나중에 못 바꿈
- 3단계가 같은 이름을 쓰도록 처음에 정함

---

**다음으로 읽을 것**

| | |
|---|---|
| 왜 이 구조인가 | [architecture.md](architecture.md) |
| 파일별 역할 | [code-map.md](code-map.md) |
| 테이블 구조 | [tables.md](tables.md) |
| 판단 기준 P1~P22 | [principles.md](principles.md) |
| 실측과 실패 기록 | [findings.md](findings.md) |
