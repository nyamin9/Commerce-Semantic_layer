...
- 기능 부족이 아니라 도구의 층위이고, dbt Core 도 같은 이유로 못 함
...
- 세 번째 줄이 현실의 탈출구이고, `semantic_mart` 를 따로 만들어 둔 이유가 여기서 드러남
- 자유롭게 SQL 을 써도
...
- 행으로 두면 값이 중복될 뿐 아니라 **진행 중인 기간이 미래 날짜를 달게 됨.** 완결 기간의 집계는 날짜가 기간의 마지막 날이어야
  하는데, 그 날이 아직 오지 않았기 때문임
- 데이터가 9/17까지일 때 그 주의 행은 9/20 을 달고 4일치만 담음
...
- `'(all)'` rollup 행을 미리 만들어 두는 것이 이 결정의 짝임
...
- 비교 컬럼은 8개인데 서로 다른 시프트 간격은 5개뿐이라(`1 DAY`·`1 WEEK`·`1 MONTH`· `1 YEAR`·`364
  DAY`) self-join 도 5번임

- **주간 비교만 364일임.** `1 YEAR` 로 시프트하면 요일이 어긋남 — 2026-03-02(월)의 1년 전은 일요일임
