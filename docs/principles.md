...
`serving_dims` 에 dimension 하나를 넣으면 `period_`·`metric_` 의 행이 곱해진다.
`daily_` 는 거의 안 커진다 — 비용은 전부 큐브에 있다.
...
**P20-1. 나란히 적힌 선언은 제3자가 대조한다.**

같은 값이 두 곳에 적히는데 한쪽이 다른 쪽을 읽을 수 없는 경우가 있다.
...
구조로 묶지 않는 이유는 방향이다. 마트가 `entities.js` 를 참조하게 만들면 상류가
하류를 읽게 되고, 마트를 다른 팀이 소유하면 결합이 조직 경계를 넘는다.
`workflows.json` 은 JSON 이라 아예 못 읽는다.
