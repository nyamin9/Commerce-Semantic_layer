...
`CUBE` 로 `'(all)'` 을 미리 만들어 두는 이유 중 하나다.
...
| **conformed 4축 + CUBE** | **1,708** | 2,811 | **480만** |
...
| 서빙 차원 | `serving_dims` 4축 + `CUBE` 의 `'(all)'` 롤업. `daily_` 는 전체 차원 (P4) |
...
원자 집계, `period_` 는 4축 CUBE 의 기간 확장, `metric_` 은 거기에 비교를 붙인 서빙 표면이다.
