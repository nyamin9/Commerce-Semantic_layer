...
기간 컬럼 4개와 비교 컬럼 8개, 각 축의 `'(all)'` 롤업이 따라온다.
SQL 파일은 늘지 않는다.
...
**`serving_dims` 밖의 축은 누계로 못 본다.** `period_`·`metric_` 이 4축으로 접혀
있어서, `category` 별 MTD 는 `daily_` 에서 구간을 잘라 합해야 한다 (P4·P15).
...
**P4. 차원 로직은 차원 안에 산다. 고유값이 많은 컬럼은 차원이 될 수 없다.**
집계 SQL 안에서 버킷을 만들지 않는다. 파생 차원은 `semantic_mart`의 `sem_dim_*`에서 만든다.

고유값이 많으면 격자가 그만큼 부풀고, **그러면 기간 롤업이 작동하지 않는다.**
`brand`(2,753개)를 차원으로 뒀을 때 2,754일을 8년으로 접어도 행이 5%밖에 줄지 않았다.

| | `brand` 포함 | `brand` 제외 |
|---|---|---|
| `daily` 행 | 187,477 | 184,598 |
| 월 단위로 접었을 때 | 186,292 | 150,837 |
| 연 단위로 접었을 때 | **178,916** | **83,320** |
...
서빙 테이블에서는 이 `NULL` 이 `'(unknown)'` 버킷이 된다 (P6-3). 비교 기준값도 그 값으로 맞물리므로 비지 않는다 (P14).
...
**P6-2. 차원은 그 entity의 grain 에서 나온 것이어야 한다.**
...
같은 이유로 지표 수식이 보는 컬럼과 차원이 같은 grain 이어야 한다 —
`units_returned` 가 `{order_item_status}` 를 세는데 차원은 헤더 상태였던 것이
그 불일치였다.

**필터로 박을 것인가 차원으로 둘 것인가**도 여기서 갈린다. 차원으로 두면 소비
시점에 거를 수 있어 **양쪽을 다 볼 수 있다.** `buyer_count` 에서 매출 인식 필터를
뺀 것이 그 예다 — 필터를 박으면 전체 구매자를 낼 방법이 없어진다 (P4).

**P6-3. 서빙 테이블의 차원에는 `NULL` 을 두지 않는다. `'(unknown)'` 버킷이 된다.**
...
| **P10-2** | 못 옮기면 스케치로 | `buyer_count` by category |
| **P10-3** | 스케치도 안 되면 | 중앙값·분위수는 daily만 만들고 롤업을 거부한다 |

**P11. `daily_`와 `metric_`은 둘 다 재집계 가능한 형태로 저장한다.**

확정(스케치 → 정수, 분자 ÷ 분모)은 저장 시점이 아니라 **소비 시점**에 한다.
확정된 값은 더 이상 롤업할 수 없으므로, 물리 테이블에 확정값을 넣으면
차원을 걷는 순간 쓸 수 없게 된다.
...
**`weekly`·`monthly`·`yearly` 는 만들지 않는다.** 완결 기간의 롤업은 같은 축에서
...
**P14-1. `_base` 는 선언된 차원 그대로 조회할 때만 유효하다. 걷으면 과소 집계된다.**

P14 는 "원래 값이 가산이면 `_base` 도 가산이므로 차원을 걷어도 살아남는다" 고 했다.
**모든 행에 `_base` 가 채워져 있을 때만 참이다.**

시프트한 기간에 **같은 차원 조합이 없으면** `_base` 는 `NULL` 이고, `SUM` 은 `NULL` 을
빼고 더한다. 차원이 잘게 쪼개져 있을수록 매칭이 드물어 누락이 커진다.
...
걷어야 한다면 **`'(all)'` 행을 읽는다** (P4). 그 축을 걷은 롤업 행이 이미 물화돼
있고, 그 행의 `_base` 는 같은 grain 에서 시프트 조인한 값이라 맞는다. 이것이
`'(all)'` 을 미리 만들어 두는 이유 중 하나다.

**P15. 기간 누계(WTD·MTD·YTD)는 채운 격자 위에 저장한다. 희소하게 만들면 무너진다.**

누계는 소비 시점 파생으로도 낼 수 있지만, 모든 소비자가 기간 경계와 스케치 병합
규칙을 각자 알아야 한다. 그것이 semantic layer 가 없앨 지식이다 (P1).

대신 **격자를 채우는 것이 조건**이다. 활동한 날에만 누계 행을 만들면 차원을 걷는
순간 대부분이 사라진다.
...
```
1) 날짜 × 차원조합 을 전부 만든다     sem_dim_date CROSS JOIN 조합
2) daily_ 를 LEFT JOIN 해 값을 붙인다   없으면 0 (가산) · NULL (스케치)
3) 그 위에 누적한다
```
...
**격자 크기는 (조합 수 × 날짜)로만 정해진다.** 원본 행 수와 무관하다. 그래서
서빙 테이블의 차원을 `serving_dims` 4축으로 좁힌다 (P4) — `category`(26) 하나만
넣어도 26배가 된다.

| | 조합 | 날짜 | 행 |
|---|---|---|---|
| 전체 7차원 | 27,749 | 2,811 | 7,800만 |
| **conformed 4축 + 롤업** | **1,708** | 2,811 | **480만** |

**P15-1. `sem_dim_date`는 격자의 날짜 뼈대다. `daily_` 의 날짜를 쓰지 않는다.**
...
`MERGE` 는 지우지 않는다. dim 속성이 바뀌면 `(record_date, 차원)` 키가 달라져 옛 행이
...
| `dim_date` | semantic layer가 소유. 변환이 아니라 축이다.<br>**`period_` 격자의 날짜 뼈대**다. `daily_` 는 조인하지 않는다 (P15-1) |
...
| 서빙 차원 | `serving_dims` 4축 + 각 축의 `'(all)'` 롤업 행. `daily_` 는 전체 차원 (P4) |
...
| `period_`·`metric_` 갱신 | 전부 `table`. 하루가 늘면 격자가 하루 늘고 364일·1년 뒤 비교 기준값까지 바뀐다 |
| clustering | **걸지 않는다.** BigQuery 권장 기준이 64 MB인데 `daily_*` 15개 실측 최대가 16.5 MB다 |

`period_` 와 `metric_` 은 한 행에 `daily` 와 누계를 함께 담는다. 셋 다 재집계 가능한
형태로 저장하며(P11), 차이는 저장 형식이 아니라 역할이다 — `daily_` 는 전체 차원의
원자 집계, `period_` 는 `serving_dims` 롤업의 기간 확장, `metric_` 은 거기에 비교를 붙인 서빙 표면이다.
