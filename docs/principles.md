...
기간 4종과 비교 4종은 따라온다. SQL 파일은 늘지 않는다.
...
4개 기간 안에서만 움직인다. 그 밖의 임의 조합은 서빙 계층의 몫이며,
...
| | `brand` 포함 | `brand` 제외 |
|---|---|---|
| `daily` | 187,477 | 184,598 |
| `monthly` | 186,292 | 150,837 |
| `yearly` | **178,916** | **83,320** |
...
기간의 시작과 끝이 모두 필요하다. 비교 조인은 `period_start`로 맞추고,
소비 시점의 기간 누계(P15)는 `as_of_date`를 기준일로 쓴다.

| period_type | period_start | as_of_date |
|---|---|---|
| `daily` | 그날 | 그날 |
| `weekly` | 주 시작일(월) | 주 종료일(일) |
| `monthly` | 월 시작일 | 월 말일 |
| `yearly` | 연 시작일 | 연 말일 |
...
- **시프트 간격은 기간마다 다를 수 있다.** `weekly`의 YoY를 `1 YEAR`로 하면
  주 시작일에 떨어지지 않아 매칭이 전부 실패한다 — 2026-03-02(월)의 1년 전은
  일요일이다. 주간 비교는 **52주(364일)** 시프트여야 같은 요일에 떨어진다
...
|---|---|
| 선언된 차원 그대로 | 유효 |
| 차원을 걷어내며 `SUM` | **쓰지 말 것.** 그 grain 에서 다시 시프트 조인한다 |

누계(P15)를 저장하지 않는 것과 같은 성격이다 — 걷으면 깨지는 값은 소비 시점에 낸다.
`_base` 를 저장해 두는 이유는 선언된 grain 에서의 조회가 흔하기 때문이지,
어느 grain 에서나 맞기 때문이 아니다.

**P15. 기간 누계(WTD·MTD·YTD)는 저장하지 않는다. 소비 시점에 파생한다.**

비율과 같은 이유다 (P12) — **파생 가능하고 롤업에서 깨진다.**

누계를 (날짜 × 차원조합)으로 물화하려면 그 기간에 활동이 없는 조합까지
격자로 채워야 한다. 채우지 않으면 쉬는 날 조합이 빠져 차원을 걷었을 때
누계가 줄어든다. 채우면 비용이 감당되지 않는다.

```
daily_order_count 실제 행 수                      116,911
mtd 격자 (월 날짜수 × 월 조합수)                1,645,816
ytd 격자 (연 날짜수 × 연 조합수)                5,119,681   ← 지표 하나에서
```

소비 시점 계산은 `daily_`가 `dt`로 파티션되어 있어 저렴하다.

```
전체 스캔                              4,501,416 B
MTD 구간만                                 20,736 B   (0.46%)
올해 MTD + 작년 같은 날 MTD                33,824 B   (0.75%)
```

Looker · MetricFlow · Cube 모두 period-to-date를 조회 시점에 전개한다.
누계는 grain이 아니라 쿼리 패턴이다 — 자연스러운 "한 행"이 없다.

**P15-1. `sem_dim_date`는 누계의 날짜 뼈대다. `daily_` 는 조인하지 않는다.**

`daily_`에는 거래가 있었던 날만 들어간다. 빈 날짜를 채우려면 (날짜 × 차원 전 조합)
격자가 필요하고, 차원이 8개인 `order_item`은 지표 하나가 수백만 행이 된다.
누계를 저장하지 않기로 한 것과 같은 계산이다.

누계(`wtd`·`mtd`·`ytd`)는 다르다. **격자의 날짜 뼈대로 `sem_dim_date` 를 쓴다.**
`daily_` 의 날짜를 쓰면 전사적으로 거래가 0인 날이 빠져 누계의 연속성이 끊긴다 —
실측으로 2,811일 중 44일이 그랬다.

그 밖의 시계열 구멍은 조회 시점에 `sem_dim_date` 를 왼쪽에 놓고 메운다.
...
`(SELECT MAX(dt) FROM self)` 는 실행 시점에야 값이 정해져서 BigQuery 가 파티션을
...
`MERGE` 는 지우지 않는다. dim 속성이 바뀌면 `(dt, 차원)` 키가 달라져 옛 행이
...
`MAX(dt)` 를 바꾸므로 `DELETE` 와 본 쿼리가 각자 계산하면 서로 다른 구간을 보고
그 사이가 중복되거나 빈다.
