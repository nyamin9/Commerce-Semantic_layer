...
스케줄 · 실행 계정 · 상류 결함 · 현재 상태. 시간이 지나면 낡는 내용을 여기 모은다.
...
`dataform compile` 이 정하는 것은 **무엇을 만들지**다. 언제 어떤 계정으로 돌릴지는
Dataform 의 release configuration 과 workflow configuration 이 정하는데, 그건 GCP
리소스라 git 에 없다 — 레포만 보고는 무엇이 언제 도는지 알 수 없다.
...
**소유 경계가 IAM 으로 강제된다** (P1). DW 에 쓸 수 없는 것이 코드 규율이 아니라 권한이다.

Dataform 서비스 에이전트에는 이 계정으로 실행할 권한이 둘 다 필요하다.
...
`serviceAccountUser` 가 없으면 **수동 실행은 되는데 스케줄 실행만 code 7 로 실패한다.**
수동 실행은 사람의 권한으로 돌기 때문에 이 차이가 드러나지 않는다.
...
`rpt_daily_funnel` · `rpt_user_cohort_retention` 은 폐기 대상이 아니다. 우리가
**의도적으로 만들지 않은 영역**이고 registry 에 `is_generated: false` 로 남아 있다.
퍼널은 아래 결함 7, 코호트는 사용자 생애주기라서 후속 작업이다.

## 5. 알려진 상류 결함

우회하지 않고 assertion 으로 감시한다. 원인은 dbt-airflow 쪽에 있다.
...
`upstream-monitoring` 워크플로가 매일 3건 실패한다. **그것이 정상이다.**
...
전부 DW 테이블만 읽으므로 본 파이프라인에 영향이 없다.
...
`dataform run` 은 `.df-credentials.json` 또는 ADC 가 필요하다. 실제 운영은 Dataform
콘솔의 서비스 계정으로 돌아가므로 로컬 실행은 편의 목적이다.

`main` 이 Dataform 이 추적하는 브랜치다. 콘솔 workspace 는 자동 동기화되지 않으므로
푸시 후 `Pull from default branch` 를 눌러야 반영된다.

### 스키마를 바꿀 때

파티션 컬럼을 바꾸면 `CREATE OR REPLACE` 가 거부된다. 테이블을 지우고 다시 만들어야
한다. 컬럼 추가·삭제만이면 `--full-refresh` 로 충분하다.
