...
//
// 조인이 실행되는 유일한 곳이다 (P5). period_ 는 여기서만 읽고 atomic fact 와
// dimension 을 다시 읽지 않는다 (P11).
//
// dimension이 가장 넓은 테이블이다. period_·metric_ 은 serving_dims 로 좁혀지므로,
// category·department 같은 dimension 이 필요하면 여기서 직접 집계한다 (P4).
//
...
Object.entries(METRICS).forEach(([name, m]) => {
  ...
  const columns = {
    ...
    [name]: sketch
      ? `${m.description} — HLL sketch(BYTES). 값을 보려면 HLL_COUNT.EXTRACT (P11)`
      : m.description,
  };
  ...
  publish(dailyName(name), {
    ...
    // 유령 행이 남지 않는다 — MERGE 를 쓸 때와 달리 dimension에 NULL 이 있어도 된다 (P6-1)
    assertions: {
