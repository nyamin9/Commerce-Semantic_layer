...
//
// 전부 table 이다 (P22). 하루가 늘면 그날의 누계가 생기고 격자도 하루 늘어난다.

const { METRICS }         = require("includes/metrics");
const { periodName, RECORD_DATE, valueColumn } = require("includes/naming");
...
Object.entries(METRICS).forEach(([name, m]) => {
  ...
  for (const d of axes) {
    columns[d] = `차원. '(all)' 은 이 축을 걷은 롤업 행, '(unknown)' 은 값이 없는 버킷 (P6-3)`;
  }
  ...
  publish(periodName(name), {
    ...
    // 키가 record_date 하나 + 차원이다. period_type 이 없어져 단순해졌다.
